...
# Scan only text files tracked by git. Synthetic CI values should avoid these
# real-world token/key signatures entirely.
if git grep -nIE \
  '-----BEGIN ([A-Z0-9 ]+ )?PRIVATE KEY-----|github_pat_[A-Za-z0-9_]{20,}|gh[pousr]_[A-Za-z0-9]{20,}|AKIA[0-9A-Z]{16}|AIza[0-9A-Za-z_-]{20,}|sk-[A-Za-z0-9_-]{20,}' \
...
for (const file of files) {
  ...
  let text;
  try {
    text = execFileSync('git', ['show', `:${file}`], { encoding: 'utf8', maxBuffer: 16 * 1024 * 1024 });
  } catch {
    continue;
  }
